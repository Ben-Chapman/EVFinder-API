...
# - Copy this file to test_<manufacturer>.py in the tests directory
# - Replace "manufacturer" throughout this file to reflect the new manufacturer's
#   details.
# - You will need to add additional logic to test the specifics for each manufacturer.
...
def _test_cassette(request):
    ...
    cassette_name = f"manufacturer-{vehicle_model}.yaml"
    with vcr.use_cassette(cassette_name):
        r = client.get("/api/inventory/manufacturer", headers=headers, params=params)
...
def test_manufacturer_inventory_response_is_json(test_cassette):
    try:
        test_cassette.json()
    except Exception:
